...
Block::Block(vector3d<double> x_, vector3d<double> y_, vector3d<double> z_)
{
    ...
    // Array initialisation. A +2 cell 'halo' is put around the 3d block, this
    // allows easy patching. Geometry
...
void
Block::fill_geometry_arrays(vector3d<double>& x_, vector3d<double>& y_, vector3d<double>& z_)
{
...
void
Block::set_block_id(int id_)
{
    id = id_;
}

void
Block::initialise_walls()
{
    // We start by assuming all block boundaries are walls, when we read the
    // patches we can then modify where necessary. k faces.
