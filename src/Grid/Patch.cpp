...
#include <stdexcept>
void
PeriodicPatch::apply(Solver& solver)
{
    ...
    for (int i = 0; i < extent.ien - extent.ist; i++) {
        for (int j = 0; j < extent.jen - extent.jst; j++) {
            for (int k = 0; k < extent.ken - extent.kst; k++) {
                ...
                if (nextDir.nexti == "+I")
                    nxpi = nxp->extent.ist + i;
                else if (nextDir.nexti == "-I")
                    nxpi = nxp->extent.ien - 1 - i;
                else if (nextDir.nexti == "+J")
                    nxpj = nxp->extent.jst + i;
                else if (nextDir.nexti == "-J")
                    nxpj = nxp->extent.jen - 1 - i;
                else if (nextDir.nexti == "+K")
                    nxpk = nxp->extent.kst + i;
                else if (nextDir.nexti == "-K")
                    nxpk = nxp->extent.ken - 1 - i;

                if (nextDir.nextj == "+I")
                    nxpi = nxp->extent.ist + j;
                else if (nextDir.nextj == "-I")
                    nxpi = nxp->extent.ien - 1 - j;
                else if (nextDir.nextj == "+J")
                    nxpj = nxp->extent.jst + j;
                else if (nextDir.nextj == "-J")
                    nxpj = nxp->extent.jen - 1 - j;
                else if (nextDir.nextj == "+K")
                    nxpk = nxp->extent.kst + j;
                else if (nextDir.nextj == "-K")
                    nxpk = nxp->extent.ken - 1 - j;

                if (nextDir.nextk == "+I")
                    nxpi = nxp->extent.ist + k;
                else if (nextDir.nextk == "-I")
                    nxpi = nxp->extent.ien - 1 - k;
                else if (nextDir.nextk == "+J")
                    nxpj = nxp->extent.jst + k;
                else if (nextDir.nextk == "-J")
                    nxpj = nxp->extent.jen - 1 - k;
                else if (nextDir.nextk == "+K")
                    nxpk = nxp->extent.kst + k;
                else if (nextDir.nextk == "-K")
                    nxpk = nxp->extent.ken - 1 - k;
                ...
                for (auto& [key, val] : b1.secondary_vars) {
                    b1.secondary_vars[key][pi][pj][pk] =
                        0.5 * (b1.secondary_vars[key][pi][pj][pk] + b2.secondary_vars[key][nxpi][nxpj][nxpk]);
                    b2.secondary_vars[key][nxpi][nxpj][nxpk] = b1.secondary_vars[key][pi][pj][pk];
                }

                // This sets the fluxes to be periodic, I don't think we need it
                // though.
                //                for (auto& [key, val]: b1.c_fluxes) {
                //                    for (int faceId=0; faceId < 3; faceId++){
                //                        b1.c_fluxes[key][faceId][i+extent.ist][j+extent.jst][k+extent.kst]
                //                        =
                //                        b2.c_fluxes[key][faceId][i+nxp->extent.ist][j+nxp->extent.jst][k+nxp->extent.kst];
                //                        if (face == 2) {
                //                            b1.c_fluxes[key][faceId][i+extent.ist][j+extent.jst][k+extent.kst-1]
                //                            =
                //                            b2.c_fluxes[key][faceId][i+nxp->extent.ist][j+nxp->extent.jst][k+nxp->extent.kst-1];
                //                        } else if (face == 5) {
                //                            b1.c_fluxes[key][faceId][i+extent.ist][j+extent.jst][k+extent.kst+1]
                //                            =
                //                            b2.c_fluxes[key][faceId][i+nxp->extent.ist][j+nxp->extent.jst][k+nxp->extent.kst+1];
...
std::string
PeriodicPatch::to_string()
{
    std::string str(std::string("Periodic: bid: ") + std::to_string(bid) + std::string(", pid: ") + std::to_string(pid) +
                    std::string(", nxbid: ") + std::to_string(nxbid) + std::string(", nxpid: ") + std::to_string(nxpid) +
                    std::string(", ") + extent.to_string() + std::string(", ") + nextDir.to_string());
    return str;
}

void
PeriodicPatch::alter_block_iteration_extent(Grid& g)
{
...
ExitPatch::to_string()
{
    std::string str(std::string("Exit: bid: ") + std::to_string(bid) + std::string(", pid: ") + std::to_string(pid) + std::string(" ") +
                    extent.to_string() + std::string(" p_exit: ") + std::to_string(p_exit));
    return str;
}

void
InletPatch::apply(Solver& solver)
{
    ...
    for (int i = extent.ist; i < extent.ien; i++) {
        for (int j = extent.jst; j < extent.jen; j++) {
            for (int k = extent.kst; k < extent.ken; k++) {
                ...
                // If this is the first iteration we don't have a ro_(n-1) so we
                // just use the current ro.
                if (solver.nstep > 0) {
...
{
    std::string str(std::string("Inlet: bid: ") + std::to_string(bid) + std::string(", pid: ") + std::to_string(pid) + std::string(" ") +
                    extent.to_string() + std::string(" ") + conditions.to_string());
    return str;
}

void
Patch::shift_patch_extent(int ishift, int jshift, int kshift)
{
    extent.ist += ishift;
    extent.jst += jshift;
    extent.kst += kshift;
