...
void
Solver::calculate_block(Block& b, SolutionParameters& sp, double delta_t)
{
    Extent e{};
...
void
Solver::set_secondary_variables(Block& b) const
{
    ...
    for (int i = b.ist; i < b.ien; i++) {
        for (int j = b.jst; j < b.jen; j++) {
            for (int k = b.kst; k < b.ken; k++) {
                ...
                double velSq   = vx[i][j][k] * vx[i][j][k] + vy[i][j][k] * vy[i][j][k] + vz[i][j][k] * vz[i][j][k];
...
void
Solver::set_mass_fluxes(Block& b, int faceId, Extent& extent)
{
    vector3d<double>&    rovx = b.primary_vars["rovx"];
    vector3d<double>&    rovy = b.primary_vars["rovy"];
    vector3d<double>&    rovz = b.primary_vars["rovz"];
    ...
    for (int i = extent.ist; i < extent.ien; i++) {
        for (int j = extent.jst; j < extent.jen; j++) {
            for (int k = extent.kst; k < extent.ken; k++) {
                ...
                // Sum the x,y,z  components of the flux through the i-face for
                // this cell.
...
void
Solver::set_convective_fluxes(Block& b, vectornd<double, 4>& flux, std::string varName, int faceId, Extent& extent)
{
    ...
    for (int i = extent.ist; i < extent.ien; i++) {
        for (int j = extent.jst; j < extent.jen; j++) {
            for (int k = extent.kst; k < extent.ken; k++) {
                ...
                // If this is momentum flux we also include the relevant PdA
                // term.
                if (varName.at(1) == 'x') {
...
void
Solver::set_timestep(double tstag)
{
...
void
Solver::sum_convective_fluxes(Block&               b,
                              vector3d<double>&    phi,
                              vectornd<double, 4>& flux,
                              ...
                              double               delta_t)
{
    ...
    for (int i = b.ist; i < b.ien - 1; i++) {
        for (int j = b.jst; j < b.jen - 1; j++) {
            for (int k = b.kst; k < b.ken - 1; k++) {
                b.store[i][j][k] = -delta_t / b.volume[i][j][k] *
                                   (flux[0][i][j][k] - flux[0][i + 1][j][k] + flux[1][i][j][k] - flux[1][i][j + 1][k] + flux[2][i][j][k] -
                                    flux[2][i][j][k + 1]);
    ...
    // Now we distribute the changes in the variable to the 8 corners of the
    // cell. Interior nodes:
    for (int i = b.ist + 1; i < b.ien - 1; i++) {
        for (int j = b.jst + 1; j < b.jen - 1; j++) {
            for (int k = b.kst + 1; k < b.ken - 1; k++) {
                phi[i][j][k] += 0.125f * (b.delta_phi[i][j][k] + b.delta_phi[i - 1][j][k] + b.delta_phi[i - 1][j - 1][k] +
                                          b.delta_phi[i][j - 1][k] + b.delta_phi[i][j][k - 1] + b.delta_phi[i - 1][j][k - 1] +
                                          b.delta_phi[i - 1][j - 1][k - 1] + b.delta_phi[i][j - 1][k - 1]);
            }
        }
    }
    ...
    for (int i = b.ist + 1; i < b.ien - 1; i++) {
        for (int k = b.kst + 1; k < b.ken - 1; k++) {
            phi[i][b.jst][k] += 0.25f * (b.delta_phi[i][b.jst][k] + b.delta_phi[i - 1][b.jst][k] + b.delta_phi[i][b.jst][k - 1] +
                                         b.delta_phi[i - 1][b.jst][k - 1]);
            phi[i][b.jen - 1][k] += 0.25f * (b.delta_phi[i][b.jen - 2][k] + b.delta_phi[i - 1][b.jen - 2][k] +
                                             b.delta_phi[i][b.jen - 2][k - 1] + b.delta_phi[i - 1][b.jen - 2][k - 1]);
        }
    }
    ...
    for (int i = b.ist + 1; i < b.ien - 1; i++) {
        for (int j = b.jst + 1; j < b.jen - 1; j++) {
            phi[i][j][b.kst] += 0.25f * (b.delta_phi[i][j][b.kst] + b.delta_phi[i - 1][j][b.kst] + b.delta_phi[i][j - 1][b.kst] +
                                         b.delta_phi[i - 1][j - 1][b.kst]);
            phi[i][j][b.ken - 1] += 0.25f * (b.delta_phi[i][j][b.ken - 2] + b.delta_phi[i - 1][j][b.ken - 2] +
                                             b.delta_phi[i][j - 1][b.ken - 2] + b.delta_phi[i - 1][j - 1][b.ken - 2]);
        }
    }
    ...
    for (int j = b.jst + 1; j < b.jen - 1; j++) {
        for (int k = b.kst + 1; k < b.ken - 1; k++) {
            phi[b.ist][j][k] += 0.25f * (b.delta_phi[b.ist][j][k] + b.delta_phi[b.ist][j - 1][k] + b.delta_phi[b.ist][j][k - 1] +
                                         b.delta_phi[b.ist][j - 1][k - 1]);
            phi[b.ien - 1][j][k] += 0.25f * (b.delta_phi[b.ien - 2][j][k] + b.delta_phi[b.ien - 2][j - 1][k] +
                                             b.delta_phi[b.ien - 2][j][k - 1] + b.delta_phi[b.ien - 2][j - 1][k - 1]);
        }
    }
...
void
Solver::set_wall_bconds(Block& b)
{
    ...
    for (int j = b.jst; j < b.jen - 1; j++) {
        for (int k = b.kst; k < b.ken - 1; k++) {
            if (b.isWall[b.ist][j][k] || b.isWall[b.ist][j + 1][k] || b.isWall[b.ist][j][k + 1] || b.isWall[b.ist][j + 1][k + 1]) {
                b.c_fluxes["mass"][0][b.ist][j][k] = 0;
            }

            if (b.isWall[b.ien - 1][j][k] || b.isWall[b.ien - 1][j + 1][k] || b.isWall[b.ien - 1][j][k + 1] ||
                b.isWall[b.ien - 1][j + 1][k + 1]) {
    ...
    for (int i = b.ist; i < b.ien - 1; i++) {
        for (int k = b.kst; k < b.ken - 1; k++) {
            if (b.isWall[i][b.jst][k] || b.isWall[i + 1][b.jst][k] || b.isWall[i][b.jst][k + 1] || b.isWall[i + 1][b.jst][k + 1]) {
                b.c_fluxes["mass"][1][i][b.jst][k] = 0;
            }

            if (b.isWall[i][b.jen - 1][k] || b.isWall[i + 1][b.jen - 1][k] || b.isWall[i][b.jen - 1][k + 1] ||
                b.isWall[i + 1][b.jen - 1][k + 1]) {
    ...
    for (int i = b.ist; i < b.ien - 1; i++) {
        for (int j = b.jst; j < b.jen - 1; j++) {
            if (b.isWall[i][j][b.kst] || b.isWall[i][j + 1][b.kst] || b.isWall[i + 1][j][b.kst] || b.isWall[i + 1][j + 1][b.kst]) {
                b.c_fluxes["mass"][2][i][j][b.kst] = 0;
            }

            if (b.isWall[i][j][b.ken - 1] || b.isWall[i][j + 1][b.ken - 1] || b.isWall[i + 1][j][b.ken - 1] ||
                b.isWall[i + 1][j + 1][b.ken - 1]) {
...
void
Solver::smooth(Block& b, vector3d<double>& phi)
{
    ...
    for (int i = b.ist + 1; i < b.ien - 1; i++) {
        for (int j = b.jst + 1; j < b.jen - 1; j++) {
            for (int k = b.kst + 1; k < b.ken - 1; k++) {
                double avg = 0.1666667f * (phi[i + 1][j][k] + phi[i - 1][j][k] + phi[i][j + 1][k] + phi[i][j - 1][k] + phi[i][j][k + 1] +
                                           phi[i][j][k - 1]);
    ...
    for (int j = b.jst; j < b.jen; j++) {
        for (int k = b.kst; k < b.ken; k++) {
            ...
            double avg = (0.2f * phi[b.ist][jp1][k] + 0.2f * phi[b.ist][jm1][k] + 0.2f * phi[b.ist][j][kp1] + 0.2f * phi[b.ist][j][km1] +
                          0.4f * phi[b.ist + 1][j][k] - 0.2f * phi[b.ist + 2][j][k]);
            b.store[b.ist][j][k] = sfm1 * phi[b.ist][j][k] + sp.sfin * avg;

            avg                      = (0.2f * phi[b.ien - 1][jp1][k] + 0.2f * phi[b.ien - 1][jm1][k] + 0.2f * phi[b.ien - 1][j][kp1] +
                   0.2f * phi[b.ien - 1][j][km1] + 0.4f * phi[b.ien - 2][j][k] - 0.2f * phi[b.ien - 3][j][k]);
    ...
    for (int i = b.ist; i < b.ien; i++) {
        for (int k = b.kst; k < b.ken; k++) {
            ...
            double avg = (0.2f * phi[ip1][b.jst][k] + 0.2f * phi[im1][b.jst][k] + 0.2f * phi[i][b.jst][kp1] + 0.2f * phi[i][b.jst][km1] +
                          0.4f * phi[i][b.jst + 1][k] - 0.2f * phi[i][b.jst + 2][k]);
            b.store[i][b.jst][k] = sfm1 * phi[i][b.jst][k] + sp.sfin * avg;

            avg                      = (0.2f * phi[ip1][b.jen - 1][k] + 0.2f * phi[im1][b.jen - 1][k] + 0.2f * phi[i][b.jen - 1][kp1] +
                   0.2f * phi[i][b.jen - 1][km1] + 0.4f * phi[i][b.jen - 2][k] - 0.2f * phi[i][b.jen - 3][k]);
    ...
    for (int i = b.ist; i < b.ien; i++) {
        for (int j = b.jst; j < b.jen; j++) {
            ...
            double avg = (0.2f * phi[ip1][j][b.kst] + 0.2f * phi[im1][j][b.kst] + 0.2f * phi[i][jp1][b.kst] + 0.2f * phi[i][jm1][b.kst] +
                          0.4f * phi[i][j][b.kst + 1] - 0.2f * phi[i][j][b.kst + 2]);
            b.store[i][j][b.kst] = sfm1 * phi[i][j][b.kst] + sp.sfin * avg;

            avg                      = (0.2f * phi[ip1][j][b.ken - 1] + 0.2f * phi[im1][j][b.ken - 1] + 0.2f * phi[i][jp1][b.ken - 1] +
                   0.2f * phi[i][jm1][b.ken - 1] + 0.4f * phi[i][j][b.ken - 2] - 0.2f * phi[i][j][b.ken - 3]);
...
void
Solver::smooth_defcorr(Block& b, vector3d<double>& phi, vector3d<double>& corr_phi, SolutionParameters& sp)
{
...
void
Solver::print_flux(vectornd<double, 4> var, int faceId, int k)
{
...
void
Solver::print_var(vector3d<double> var, int k)
{
...
void
Solver::print_Ax(vector3d<Cell> var, int k)
{
    for (int j = var.shape()[2] - 1; j >= 0; j--) {
        for (int i = 0; i < var.shape()[1]; i++) {
